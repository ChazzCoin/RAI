...
from rai.ingest.utilities.IngestModels import IngestBrief, TextLineDetail, TextLineClassification
...
def convert_pdf_to_images(pdf_input: Union[str, bytes], dpi: int = 200, fmt: str = "PNG") -> List[bytes]:
    ...
    for page in pages:
        try:
            ...
            print(f"Failed to convert page to {fmt} bytes: {e}")
...
class IngestPdfMiner(TextProcessor):
    ...
    def run(self):
        ...
        Returns: A dictionary (self.book) with keys as page numbers and values as extracted page data.
        """
        ...
        def attempt_image_extraction(img:bytes):
            try:
