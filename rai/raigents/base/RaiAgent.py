...
from rai.raigents.base.RedisAgent import RaiAgentRedis
...
class RaiAgentQueue(ABC, RaiAi, TextProcessor):
    def __init__(self):
        super().__init__()
    ...
    def text(name: str, user_prompt: str, system_prompt: str):
        return RaiBaseTextAgent.generate(name=name, user_prompt=user_prompt, system_prompt=system_prompt)

    @staticmethod
    def image(name: str, image: str):
        return RaiBaseImageAgent.generate(name=name, image=image)
    ...
    def setup_cache(self):
        ...
        if self.use_redis:
            try:
                ...
                self.redis_cache = RaiAgentRedis(db=self.redis_db, queue_name=self.queue_name)
