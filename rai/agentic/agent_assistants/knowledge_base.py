...
class KnowledgeTool(ToolEngine):
    ...
    def get_tools(self) -> List[dict[str, Any]]:
        return [
            self.get_tool('delete_document'),
            self.get_tool('update_document'),
            self.get_tool('get_all_documents'),
            self.get_tool('find_documents'),
            self.get_tool('browse_documents'),
            self.get_tool('get_document_by_id'),
            self.get_tool('query_documents_by_specific_date'),
            self.get_tool('query_documents_today'),
            self.get_tool('query_documents_by_date_range')
        ]

    async def get_current_state(self) -> ToolResult:
        return ToolResult(
            output="We are getting the current state of the knowledge assistant",
        )
    ...
    def work(cls, prefix:str, user_prompt:str):
        ...
        results = self.self_navigation(user_prompt)
    ...
    #     self.log_voice(f"Document Manager initialized with prefix: {self.prefix}")
    ...
    def find_documents(self, query_text: str = "") -> List[StoreDocument]:
        ...
        """
        self.log_voice(f"Finding documents with query: '{query_text}'.")
        ...
        self.log_voice(f"Found {len(parsed_results)} documents matching query.")
    ...
    def browse_documents(self, limit: int = None) -> List[dict]:
        ...
        """
        self.log_voice(f"Attempting to retrieve documents.")
        result = self.rStore().get(self.prefix, combined=True)
        if not result:
            self.log_thought("No documents found in the collection.")
            return []
        if limit is not None:
            if limit == 0: limit = 100
            self.log_voice(f"Returning [ {limit} ] retrieved documents.")
            result = result[:limit]
        self.log_voice(f"Returning [ {len(result)} ] retrieved documents.")
        return result
    def get_document_by_id(self, doc_id) -> List[dict]:
        ...
        for doc in documents:
            if doc["id"] == doc_id:
                return [doc]
        self.log_voice(f"Document with ID '{doc_id}' not found.")
        return []
    def query_documents_by_specific_date(self, query_date: Union[str, datetime]) -> list[StoreDocument]:
        ...
        self.log_voice(f"Querying documents on {query_date.date()} (timestamps {start_ts} to {end_ts})")
    ...
    def query_documents_today(self) -> list[StoreDocument]:
        ...
        self.log_voice(f"Querying documents for today's date: {today.date()}")
        return self.query_documents_by_specific_date(today)
    def query_documents_by_date_range(self, start_date: Union[str, datetime], end_date: Union[str, datetime]) -> list[StoreDocument]:
        """
...
if __name__ == "__main__":
    ...
    looper.run_until_complete(KnowledgeTool().self_navigation(request="how many documents do I have? prefix is 'referral2025.1'"))
