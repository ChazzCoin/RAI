...
from rai.data.extraction.parsers.PDF.OCR import OCR
from rai.data.extraction.parsers.PDF.Recognizer import Recognizer
from rai.data.extraction.parsers.PDF.LayoutRecognizer import LayoutRecognizer
from rai.data.extraction.parsers.PDF.TableStructureRecognizer import TableStructureRecognizer
from rai.data import DataUtilities as rag_tokenizer
from rai.data.DataUtilities import get_project_base_directory
    ...
    def __images__(self, fnm, zoomin=3, page_from=0,
        ...
        try:
            ...
            self.page_images_x2 = [p.to_image(resolution=72 * zoomin * 2).annotated for i, p in
                                   enumerate(self.pdf.pages[page_from:page_to])]
            try:
                self.page_chars = [[{**c, 'top': c['top'], 'bottom': c['bottom']} for c in page.dedupe_chars().chars if
                                    self._has_color(c)] for page in self.pdf.pages[page_from:page_to]]
...
if __name__ == "__main__":
    pdfer = RAGFlowPdfParser()
    data = pdfer("/Users/chazzromeo/Desktop/Proverbs-Unsorted/Babylonian Records.pdf")
    if data:
        for item in data:
            print(item)
    print("data")